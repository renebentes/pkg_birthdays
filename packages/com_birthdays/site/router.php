...
function BirthdaysBuildRoute(&$query)
{
	...
	// Are we dealing with an birthdays that is attached to a menu item?
	...
	if (isset($view) and ($view == 'category' or $view == 'birthdays'))
	{
		if ($mId != intval($query['id']) || $mView != $view)
		{
			if ($view == 'birthdays' && isset($query['catid']))
			{
				$catid = $query['catid'];
			}
...
function BirthdaysParseRoute($segments)
{
	...
	// Standard routing for birthdays.
	if (!isset($item))
	{
	...
	$categories = JCategories::getInstance('Birthdays')->get($id)->getChildren();
	$vars['catid'] = $id;
	$vars['id'] = $id;
	$found = 0;
	...
	foreach ($segments as $segment)
	{
		...
		if ($found == 0)
		{
			...
			{
				$db = JFactory::getDBO();
				$query = 'SELECT id FROM #__birthdays WHERE catid = ' . $vars['catid'] . ' AND alias = ' . $db->Quote($segment);
				$db->setQuery($query);
				$nid = $db->loadResult();
			}
			else
